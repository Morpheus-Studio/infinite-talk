...
# Build and push InfiniteTalk Docker image (with weights) to Docker Hub

set -e

VERSION=0.1
IMAGE_NAME=infinite-talk-with-weights
DOCKER_HUB_USER=jonathan28alkalay

FULL_IMAGE_NAME=${IMAGE_NAME}:${VERSION}
DOCKER_HUB_IMAGE=${DOCKER_HUB_USER}/${IMAGE_NAME}:${VERSION}

echo "Building Docker image (with weights): ${FULL_IMAGE_NAME}"
docker build --platform=linux/amd64 -f Dockerfile.with_weights -t ${FULL_IMAGE_NAME} .
